...
export class MonitorService {
  ...
  public async getEchonetClassFromDeviceWithIpAddress(
    ipAddress: string,
    echonetClass: EchonetClass,
  ): Promise<EchonetResponse> {
    const device = this.discoveryService.findEchonetDeviceFromIpAddress(
      ipAddress,
    );
    return new Promise((resolve, reject) => {
      this.echonet.getPropertyValue(
        device.address,
        device.eoj[0],
        echonetClass,
        (error, response) => {
  ...
  public async listEchonetDeviceInfo(): Promise<EchonetDeviceInfo[]> {
    ...
    const currentRoomTemperature = await Promise.all(
      currentDevices.map(device =>
        this.getEchonetClassFromDeviceWithIpAddress(
          device.address,
          EchonetClass.ROOM_TEMPERATURE,
        ),
      ),
    );
    const currentOutsideTemperature = await Promise.all(
      currentDevices.map(device =>
        this.getEchonetClassFromDeviceWithIpAddress(
          device.address,
          EchonetClass.OUTDOOR_TEMPERATURE,
        ),
      ),
    );
  ...
  public async getEchonetDeviceDetail(
    ipAddress: string,
  ): Promise<EchonetDeviceDetail> {
    const device = this.discoveryService.findEchonetDeviceFromIpAddress(
      ipAddress,
    );
    const currentRoomTemperature = await this.getEchonetClassFromDeviceWithIpAddress(
      device.address,
      EchonetClass.ROOM_TEMPERATURE,
    );
    const currentOutsideTemperature = await this.getEchonetClassFromDeviceWithIpAddress(
      device.address,
      EchonetClass.OUTDOOR_TEMPERATURE,
    );
    const currentStatus = await this.getEchonetClassFromDeviceWithIpAddress(
      device.address,
      EchonetClass.OPERATION_STAT,
    );
