...
export class ControlService {
  ...
  public changeOperationStatusOfAllDevices(status: ActionType) {
  ...
  public changeTemperatureOfAllDevices(temperature: number) {
    const devices = this.discoveryService.getCurrentDevice();
  ...
  private setOperationStatusOfDevices(
    ...
    devices.forEach(device => {
      this.echonet.setPropertyValue(
        device.address,
        device.eoj[0],
        EchonetClass.OPERATION_STAT,
        { status: status === ActionType.OPEN },
      );
    });
  }
